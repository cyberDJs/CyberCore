...
Work block: `WB-0028`
Review state: `READY_FOR_DECISION`
Recommendation: `ACCEPT`

## Executive verdict

ADR-0006 is ready for an explicit operator decision. The recommended decision is **ACCEPT**.
...
The currently unknown InterServer deployment mechanism (`SSH/rsync`, `SFTP`, or provider-native) does not block acceptance. It remains an implementation preflight gate before any `staging_apply` action.
...
Canonical review base: `main@4ca00fb7e1a6b618746afb2045e230a1763256e4`.

## Decision-quality assessment

### Distinct options

1. **Accept ADR-0006** — establish staging-only self-deployment as the governed first step. **Recommended.**
2. **Defer ADR-0006** — keep all self-deployment design as non-authoritative draft material. Lower immediate governance commitment, but creates ambiguity for the next implementation slice.
3. **Reject ADR-0006** — prohibit the current self-deployment direction and require a new architecture decision before further work. No evidence currently justifies this.

Direct production-first automation is not treated as a viable option because it conflicts with existing CyberCore governance and explicit production-mutation boundaries.
...
These unknowns block **remote execution**, not the staging-first architectural decision.
...
## Acceptance criteria for ADR-0006

The architecture decision can be accepted when the operator agrees that:
...
All six are already represented consistently in the current repository evidence.
...
Recommended next slice: a **disabled/manual staging workflow + local manifest/target validator** that supports `plan_only` and `dry_run` first.

`staging_apply` must remain fail-closed until target identity, secret aliases, deployment capability, rollback, effect verifier, and a fresh explicit remote-write authorization are all verified.

## Rollback / exit

If ADR-0006 is later found unsuitable, do not silently rewrite history. Mark it `Deprecated` or `Superseded` and create a replacement ADR. Until any remote-write slice is separately authorized, accepting or rejecting this ADR has no production effect.

## Authority gate

`READY_FOR_DECISION` is a review result only. Changing ADR-0006 from `Proposed` to `Accepted` requires explicit Jan Kočí authorization.
