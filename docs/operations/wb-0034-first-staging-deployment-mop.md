...
- capture manifest and readiness from their source paths exactly once;
- parse the captured readiness bytes only to identify the bounded evidence reference, then capture that evidence file exactly once;
- materialize those captured bytes into a private temporary packet snapshot and run the existing manifest/readiness/evidence validators against that private snapshot, not against mutable source paths;
- verify the private snapshot bytes still equal the originally captured bytes after validation;
- use the already-captured manifest/readiness mappings for all combined cross-document checks; never re-read their original paths during the same packet validation;
...
- use `scripts/validate_wb0034_packet.py --repo-root <checked-out-main> --artifact-dir <exact-two-file-artifact-dir>` only as a diagnostic/operator evidence check; it does not authorize or perform upload and cannot carry validated bytes across a process boundary;
- require the authoritative future write runner to call `validate_first_write_packet(...)` in the same process immediately before its approved upload and retain the returned `upload_input`;
...
- require each evidence artifact SHA-256 to match the exact no-follow-read local bytes retained in `upload_input`;
...
Only after fresh authorization and a passing **in-process** final packet:

1. keep the returned `FirstWriteUploadInput` object in the same process; do not serialize it to ordinary evidence and do not reopen local artifact or packet source paths;
2. connect using the protocol and deploy-identity scope already bound into that validated upload input and the separately approved credential alias;
3. resolve the target base path to `/home/eimyherr/domains/staging.eimyherrer.com/public_html`;
4. fail if the resolved base differs from the approved canonical staging path;
5. use exactly the direct-child destination carried by `upload_input`;
6. perform a non-following metadata check on that destination and fail unless it is absent;
7. reject any symlink encountered from the canonical staging root downward;
8. immediately before creation, re-resolve the parent and require it to remain the canonical staging root;
9. create only that one unique directory with fail-if-exists semantics;
10. upload only the immutable `index.html` and `cybercore-version.json` byte payloads carried by `upload_input`, with no-overwrite semantics;
11. do not reopen the original artifact directory to obtain upload bytes;
12. disconnect;
13. run effect verification;
14. create a sanitized evidence receipt containing hashes and identities, never the artifact payloads or secret values.
...
- uploaded artifact hashes equal the hashes carried by the validated in-process upload input;
...
- final validation did not produce a sealed in-process `upload_input` containing both approved immutable byte payloads;
- an uploader would reopen artifact or packet source paths instead of consuming the validated `upload_input` bytes;
