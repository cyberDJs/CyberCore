...
- loads the credential only after input and authorization gates pass;
- verifies TLS, protected passive mode, and FTPS root `/`;
- rejects an unexpected target type;
- enters only the exact sealed destination;
- rejects any unexpected entry or non-file entry;
...
- removes only the exact sealed run directory after proving it empty;
...
- exact two-artifact deletion and exact directory removal;
...
- endpoint is `staging.eimyherrer.com`;
