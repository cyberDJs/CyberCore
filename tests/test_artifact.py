...
def kwargs():
    return dict(
        artifact_id="WB-0010-test-artifact",
        version="1.0.0",
        publisher_id="cyberdjs",
        publisher_name="CyberDJS",
        runtime_compatibility=">=0.1.0,<0.2.0",
        risk="low",
        title="Test",
        description="Test",
        created_at=FIXED,
...
def test_reproducible(tmp_path: Path):
    src = source(tmp_path)
    a = build_artifact(src, tmp_path / "a", **kwargs())
    b = build_artifact(src, tmp_path / "b", **kwargs())
