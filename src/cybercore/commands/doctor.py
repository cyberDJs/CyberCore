...
def _command_check(command: str) -> CheckResult:
    ...
    return CheckResult(
        command,
        CheckState.OK if path else CheckState.ERROR,
...
def run_doctor(paths: RuntimePaths) -> list[CheckResult]:
    results = [
        CheckResult(
            "python",
            CheckState.OK if sys.version_info >= (3, 11) else CheckState.ERROR,
            sys.version.split()[0],
        ),
        _command_check("git"),
    ...
    results.append(
        CheckResult(
            "repository",
            CheckState.OK if git_dir.is_dir() else CheckState.ERROR,
            str(paths.repo),
        )
    )
    results.append(
        CheckResult(
            "exchange-home",
            CheckState.OK if paths.exchange_home.is_dir() else CheckState.WARN,
            str(paths.exchange_home),
        )
    )
    ...
    results.append(
        CheckResult(
            "exchange-agent",
            CheckState.OK if agent.is_file() else CheckState.WARN,
            str(agent),
        )
    )
    if git_dir.is_dir():
        ...
        results.append(
            CheckResult(
                "working-tree",
                CheckState.OK if clean else CheckState.WARN,
                "clean" if clean else "contains local changes",
            )
        )
    return results
